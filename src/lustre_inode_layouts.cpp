...
//
// lustre_inode_layouts.cpp
//
// Internal fused table function for inode/layout joins
//===----------------------------------------------------------------------===//
...
static constexpr idx_t INODE_LAYOUT_EXACT_LOOKUP_BATCH_SIZE = 512;
...
struct LustreInodeLayoutsFilter {
	...
	bool HasAnyFilter() const {
		return (inode_filters && inode_filters->HasFilters()) ||
		       ...
		       (inode_device_predicate && inode_device_predicate->HasPredicate()) || !layout_filters.empty();
	}

	bool UseSequentialScan() const {
		return lookup_fids.empty() || lookup_fids.size() > INODE_LAYOUT_SEQ_SCAN_THRESHOLD;
	}
	...
	void RebuildLookupFIDs() {
		...
		if (has_inode_fids && has_layout_fids) {
			lookup_fids.reserve(MinValue(inode_fid_filter->fid_values.size(), layout_fid_filter->fid_values.size()));
	...
	Value GetLayoutValue(idx_t actual_column_idx, const LustreInodeLayoutRow &row, const string &device_path) const {
		switch (actual_column_idx) {
		case 15:
			return Value(row.layout_fid.ToString());
		case 16:
	...
	bool Matches(const LustreInodeLayoutRow &row, const string &device_path) const {
		...
		if (layout_fid_filter) {
			if (!layout_fid_filter->ContainsFID(row.layout_fid)) {
				return false;
			}
			if (layout_fid_filter->RequiresGenericEvaluation() && !layout_fid_filter->EvaluateFID(row.layout_fid)) {
				return false;
			}
		}
		for (const auto &entry : layout_filters) {
			if (!EvaluateTableFilterValue(*entry.second, GetLayoutValue(entry.first, row, device_path))) {
				return false;
			}
		}
		return true;
	...
	static unique_ptr<LustreInodeLayoutsFilter> Create(const TableFilterSet *filters, const vector<idx_t> &column_ids) {
		auto result = make_uniq<LustreInodeLayoutsFilter>();
		...
		result->layout_fid_filter = FIDOnlyFilter::Create(filters, column_ids, 15);
		...
		if (filters) {
			for (const auto &entry : filters->filters) {
				...
				if (actual_column_idx >= 16 && actual_column_idx <= 31) {
...
struct LustreInodeLayoutsGlobalState : public GlobalTableFunctionState {
	vector<string> device_paths;
	vector<idx_t> column_ids;
	MDTScanConfig scan_config;
	unique_ptr<LustreInodeLayoutsFilter> filters;
...
struct LustreInodeLayoutsLocalState : public LocalTableFunctionState {
	...
	vector<LustreInodeLayoutRow> pending_results;
	idx_t pending_results_idx = 0;

	LustreInodeLayoutsLocalState() {
		scanner = make_uniq<MDTScanner>();
	}
};

const vector<string> &LustreInodeLayoutsFunction::GetColumnNames() {
	return INODE_LAYOUT_COLUMN_NAMES;
}

const vector<LogicalType> &LustreInodeLayoutsFunction::GetColumnTypes() {
	return INODE_LAYOUT_COLUMN_TYPES;
}

static unique_ptr<FunctionData> LustreInodeLayoutsBindSingle(ClientContext &context, TableFunctionBindInput &input,
	...
	names = LustreInodeLayoutsFunction::GetColumnNames();
	return_types = LustreInodeLayoutsFunction::GetColumnTypes();
	return std::move(result);
}

static unique_ptr<FunctionData> LustreInodeLayoutsBindMulti(ClientContext &context, TableFunctionBindInput &input,
                                                            vector<LogicalType> &return_types, vector<string> &names) {
	auto result = make_uniq<LustreQueryBindData>();
	...
	if (result->device_paths.empty()) {
		throw BinderException("lustre_inode_layouts_internal requires at least one device path");
	}
	ParseNamedParameters(input.named_parameters, result->scan_config);
	EstimateCardinality(*result);
	names = LustreInodeLayoutsFunction::GetColumnNames();
...
static bool InitializeInodeLayoutsDevice(LustreInodeLayoutsGlobalState &gstate, idx_t device_idx) {
...
static unique_ptr<GlobalTableFunctionState> LustreInodeLayoutsInitGlobal(ClientContext &context,
                                                                         TableFunctionInitInput &input) {
	auto &bind_data = input.bind_data->Cast<LustreQueryBindData>();
	auto result = make_uniq<LustreInodeLayoutsGlobalState>();
	result->device_paths = bind_data.device_paths;
	result->column_ids = input.column_ids;
	result->scan_config = bind_data.scan_config;
	result->filters = LustreInodeLayoutsFilter::Create(input.filters.get(), input.column_ids);
	...
	if (!result->device_paths.empty()) {
		InitializeInodeLayoutsDevice(*result, 0);
	}
	return std::move(result);
}

static unique_ptr<LocalTableFunctionState> LustreInodeLayoutsInitLocal(ExecutionContext &context,
                                                                       TableFunctionInitInput &input,
                                                                       GlobalTableFunctionState *global_state) {
	return make_uniq<LustreInodeLayoutsLocalState>();
}
...
static void WriteInodeLayoutColumn(Vector &vec, idx_t col_idx, idx_t row_idx, const LustreInodeLayoutRow &row,
	...
	case 14:
		...
		break;
	case 15: {
		auto fid_str = row.layout_fid.ToString();
		FlatVector::GetData<string_t>(vec)[row_idx] = StringVector::AddString(vec, fid_str);
		break;
	}
	case 16:
...
static void ResetInodeLayoutsBlockGroupState(LustreInodeLayoutsLocalState &lstate) {
...
static bool ClaimNextInodeLayoutsBlockGroup(LustreInodeLayoutsGlobalState &gstate,
                                            LustreInodeLayoutsLocalState &lstate) {
...
static bool EnsureInodeLayoutsScanner(LustreInodeLayoutsGlobalState &gstate, LustreInodeLayoutsLocalState &lstate,
	...
	if (lstate.scanner_initialized) {
		...
		ResetInodeLayoutsBlockGroupState(lstate);
	...
	if (!gstate.device_initialized.load()) {
		...
		if (!gstate.device_initialized.load()) {
			if (!InitializeInodeLayoutsDevice(gstate, dev_idx)) {
				return false;
			}
		}
	}
...
static bool ExecuteExactFIDPath(LustreInodeLayoutsGlobalState &gstate, LustreInodeLayoutsLocalState &lstate,
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		while (lstate.pending_results_idx < lstate.pending_results.size() && output_count < STANDARD_VECTOR_SIZE) {
			...
			for (idx_t i = 0; i < gstate.column_ids.size(); i++) {
				WriteInodeLayoutColumn(output.data[i], gstate.column_ids[i], output_count, row, current_device);
			}
		...
		lookup_batch.reserve(INODE_LAYOUT_EXACT_LOOKUP_BATCH_SIZE);

		while (lookup_batch.size() < INODE_LAYOUT_EXACT_LOOKUP_BATCH_SIZE) {
		...
		for (auto &entry : lookup_batch) {
			...
			if (!lstate.scanner->ReadInodeLayouts(entry.ino, inode, layouts, gstate.scan_config)) {
				continue;
			}

			for (auto &layout : layouts) {
				LustreInodeLayoutRow row;
				row.inode = inode;
				row.layout_fid = inode.fid;
...
static bool ExecuteSequentialPath(LustreInodeLayoutsGlobalState &gstate, LustreInodeLayoutsLocalState &lstate,
                                  DataChunk &output, idx_t &output_count) {
	const auto &current_device = lstate.initialized_device_path;
	while (output_count < STANDARD_VECTOR_SIZE) {
		if (!ClaimNextInodeLayoutsBlockGroup(gstate, lstate)) {
			return false;
		}

		while (output_count < STANDARD_VECTOR_SIZE) {
			LustreInodeLayoutRow row;
			if (!lstate.scanner->GetNextInodeLayout(row, gstate.scan_config, lstate.block_group_max_ino)) {
				lstate.block_group_active = false;
...
static void LustreInodeLayoutsExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &gstate = data_p.global_state->Cast<LustreInodeLayoutsGlobalState>();
	auto &lstate = data_p.local_state->Cast<LustreInodeLayoutsLocalState>();
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		if (!EnsureInodeLayoutsScanner(gstate, lstate, gstate.use_sequential_scan)) {
			break;
		}
...
static unique_ptr<NodeStatistics> LustreInodeLayoutsCardinality(ClientContext &context,
...
TableFunction LustreInodeLayoutsFunction::GetFunction(bool multi_device) {
	TableFunction func("lustre_inode_layouts_internal",
	                   {multi_device ? LogicalType::LIST(LogicalType::VARCHAR) : LogicalType::VARCHAR},
	                   LustreInodeLayoutsExecute,
	                   multi_device ? LustreInodeLayoutsBindMulti : LustreInodeLayoutsBindSingle,
	                   LustreInodeLayoutsInitGlobal, LustreInodeLayoutsInitLocal);
	...
	func.cardinality = LustreInodeLayoutsCardinality;
	return func;
}

TableFunctionSet LustreInodeLayoutsFunction::GetFunctionSet() {
	TableFunctionSet set("lustre_inode_layouts_internal");
