...
// lustre_layouts.cpp
//
// Table function implementation for querying Lustre MDT layout components.
// Requires a WHERE clause with fid filter; uses OI lookup for targeted reads.
//===----------------------------------------------------------------------===//
...
static constexpr idx_t LAYOUT_EXACT_LOOKUP_BATCH_SIZE = 512;
static constexpr idx_t LAYOUT_SEQ_SCAN_THRESHOLD = 8192;
...
static unique_ptr<FunctionData> LustreLayoutsBindSingle(ClientContext &context, TableFunctionBindInput &input,
	...
	names = LAYOUT_COLUMN_NAMES;
	return_types = LAYOUT_COLUMN_TYPES;
...
static unique_ptr<FunctionData> LustreLayoutsBindMulti(ClientContext &context, TableFunctionBindInput &input,
	...
	if (result->device_paths.empty()) {
		throw BinderException("lustre_layouts requires at least one device path");
	}
	ParseNamedParameters(input.named_parameters, result->scan_config);
	names = LAYOUT_COLUMN_NAMES;
	return_types = LAYOUT_COLUMN_TYPES;
...
static void WriteLayoutRow(DataChunk &output, idx_t row_idx, const vector<idx_t> &column_ids, const LustreFID &fid,
	...
	for (idx_t i = 0; i < column_ids.size(); i++) {
		...
		switch (column_ids[i]) {
		case static_cast<idx_t>(LayoutColumnIdx::FID): {
			auto fid_str = fid.ToString();
			FlatVector::GetData<string_t>(vec)[row_idx] = StringVector::AddString(vec, fid_str);
			break;
		}
		case static_cast<idx_t>(LayoutColumnIdx::COMP_INDEX):
			FlatVector::GetData<uint32_t>(vec)[row_idx] = comp.comp_index;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::COMP_ID):
			FlatVector::GetData<uint32_t>(vec)[row_idx] = comp.comp_id;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::MIRROR_ID):
			FlatVector::GetData<uint16_t>(vec)[row_idx] = comp.mirror_id;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::COMP_FLAGS):
			FlatVector::GetData<uint32_t>(vec)[row_idx] = comp.comp_flags;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::EXTENT_START):
			FlatVector::GetData<uint64_t>(vec)[row_idx] = comp.extent_start;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::EXTENT_END):
			FlatVector::GetData<uint64_t>(vec)[row_idx] = comp.extent_end;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::PATTERN):
			FlatVector::GetData<uint32_t>(vec)[row_idx] = comp.pattern;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::STRIPE_SIZE):
			FlatVector::GetData<uint32_t>(vec)[row_idx] = comp.stripe_size;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::STRIPE_COUNT):
			FlatVector::GetData<uint16_t>(vec)[row_idx] = comp.stripe_count;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::STRIPE_OFFSET):
			FlatVector::GetData<uint16_t>(vec)[row_idx] = comp.stripe_offset;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::POOL):
			if (!comp.pool_name.empty()) {
				FlatVector::GetData<string_t>(vec)[row_idx] = StringVector::AddString(vec, comp.pool_name);
			} else {
				FlatVector::SetNull(vec, row_idx, true);
			}
			break;
		case static_cast<idx_t>(LayoutColumnIdx::DSTRIPE_COUNT):
			FlatVector::GetData<uint8_t>(vec)[row_idx] = comp.dstripe_count;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::CSTRIPE_COUNT):
			FlatVector::GetData<uint8_t>(vec)[row_idx] = comp.cstripe_count;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::COMPR_TYPE):
			FlatVector::GetData<uint8_t>(vec)[row_idx] = comp.compr_type;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::COMPR_LVL):
			FlatVector::GetData<uint8_t>(vec)[row_idx] = comp.compr_lvl;
			break;
		case static_cast<idx_t>(LayoutColumnIdx::DEVICE):
...
static void ResetLayoutsBlockGroupState(LustreLayoutsLocalState &lstate) {
...
static bool InitializeLayoutsDevice(LustreLayoutsGlobalState &gstate, idx_t dev_idx) {
...
static bool EnsureLayoutsLocalScanner(LustreLayoutsGlobalState &gstate, LustreLayoutsLocalState &lstate,
	...
	if (lstate.scanner_initialized) {
		...
		ResetLayoutsBlockGroupState(lstate);
	...
	if (!gstate.device_initialized.load()) {
		...
		if (!gstate.device_initialized.load()) {
			if (!InitializeLayoutsDevice(gstate, dev_idx)) {
				return false;
			}
		}
	}
...
static bool ClaimNextLayoutsBlockGroup(LustreLayoutsGlobalState &gstate, LustreLayoutsLocalState &lstate) {
...
static bool MatchesLayoutFIDPredicate(const FIDOnlyFilter &filter, const LustreFID &fid) {
...
static bool ExecuteExactFIDPath(LustreLayoutsGlobalState &gstate, LustreLayoutsLocalState &lstate, DataChunk &output,
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		...
		lookup_batch.reserve(LAYOUT_EXACT_LOOKUP_BATCH_SIZE);

		while (lookup_batch.size() < LAYOUT_EXACT_LOOKUP_BATCH_SIZE) {
		...
		for (auto &entry : lookup_batch) {
			...
			if (!lstate.scanner->ReadInodeLayouts(entry.ino, actual_fid, components)) {
				continue;
			}

			for (auto &comp : components) {
				LustreLayoutsGlobalState::PendingRow row;
				row.fid = actual_fid;
				row.component = std::move(comp);
...
static bool ExecuteSequentialPath(LustreLayoutsGlobalState &gstate, LustreLayoutsLocalState &lstate, DataChunk &output,
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		...
		if (!ClaimNextLayoutsBlockGroup(gstate, lstate)) {
			return false;
		}
		...
		while (output_count < STANDARD_VECTOR_SIZE) {
			LustreFID fid;
			std::vector<LustreLayoutComponent> components;
			if (!lstate.scanner->GetNextInodeLayouts(fid, components, gstate.scan_config, lstate.block_group_max_ino)) {
				lstate.block_group_active = false;
				gstate.active_block_groups.fetch_sub(1);
				break;
			}
			if (!MatchesLayoutFIDPredicate(filter, fid)) {
				continue;
			}
...
static void LustreLayoutsExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		if (!EnsureLayoutsLocalScanner(gstate, lstate, gstate.use_sequential_scan)) {
			break;
		}
...
TableFunctionSet LustreLayoutsFunction::GetFunctionSet() {
	...
	TableFunction multi_func("lustre_layouts", {LogicalType::LIST(LogicalType::VARCHAR)}, LustreLayoutsExecute,
	                         LustreLayoutsBindMulti, LustreLayoutsInitGlobal, LustreLayoutsInitLocal);
	SetLayoutCommonProperties(multi_func);
