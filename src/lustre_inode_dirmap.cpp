...
static void WriteRow(DataChunk &output, idx_t row, const vector<idx_t> &cols, const InodeDirMapPendingRow &r,
	...
	for (idx_t i = 0; i < cols.size(); i++) {
		...
		switch (cols[i]) {
		case 0:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetFID(vec, i, r.inode.fid);
			break;
		case 1:
			FlatVector::GetData<uint64_t>(vec)[row] = r.inode.ino;
			break;
		case 2:
			FlatVector::GetData<string_t>(vec)[row] = StringVector::AddString(vec, FileTypeToString(r.inode.type));
			break;
		case 3:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.mode;
			break;
		case 4:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.nlink;
			break;
		case 5:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.uid;
			break;
		case 6:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.gid;
			break;
		case 7:
			FlatVector::GetData<uint64_t>(vec)[row] = r.inode.size;
			break;
		case 8:
			FlatVector::GetData<uint64_t>(vec)[row] = r.inode.blocks;
			break;
		case 9:
			FlatVector::GetData<timestamp_t>(vec)[row] = Timestamp::FromEpochSeconds(r.inode.atime);
			break;
		case 10:
			FlatVector::GetData<timestamp_t>(vec)[row] = Timestamp::FromEpochSeconds(r.inode.mtime);
			break;
		case 11:
			FlatVector::GetData<timestamp_t>(vec)[row] = Timestamp::FromEpochSeconds(r.inode.ctime);
			break;
		case 12:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.projid;
			break;
		case 13:
			FlatVector::GetData<uint32_t>(vec)[row] = r.inode.flags;
			break;
		case 14:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetString(vec, i, dev);
			break;
		case 15:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetFID(vec, i, r.dir_fid);
			break;
		case 16:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetFID(vec, i, r.parent_fid);
			break;
		case 17:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetString(vec, i, r.dir_device);
			break;
		case 18:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetString(vec, i, r.parent_device);
			break;
		case 19:
			FlatVector::GetData<uint32_t>(vec)[row] = r.master_mdt_index;
			break;
		case 20:
			FlatVector::GetData<uint32_t>(vec)[row] = r.stripe_index;
			break;
		case 21:
			FlatVector::GetData<uint32_t>(vec)[row] = r.stripe_count;
			break;
		case 22:
			FlatVector::GetData<uint32_t>(vec)[row] = r.hash_type;
			break;
		case 23:
			FlatVector::GetData<uint32_t>(vec)[row] = r.layout_version;
			break;
		case 24:
			FlatVector::GetData<string_t>(vec)[row] = string_cache.GetString(vec, i, r.source);
			break;
		default:
			break;
		}
...
static void GenerateRows(const LustreInode &inode, const LustreFID &fid, const LustreLMV &lmv,
	...
	if (lmv.IsMaster()) {
		...
		for (auto &entry : dir_entries) {
			...
			if (bp == std::string::npos || bp + 1 >= entry.name.size() || entry.name[bp + 1] != ':')
				continue;
			LustreFID shard_fid;
			if (!LustreFID::FromString(entry.name.substr(0, bp + 1), shard_fid))
				continue;
			char *end = nullptr;
			unsigned long idx = strtoul(entry.name.c_str() + bp + 2, &end, 10);
			if (end == entry.name.c_str() + bp + 2 || idx >= lmv.lmv_stripe_count)
				continue;
...
static void Execute(ClientContext &ctx, TableFunctionInput &data_p, DataChunk &output) {
	...
	while (cnt < STANDARD_VECTOR_SIZE) {
		...
		if (!EnsureScanner(g, l, g.use_sequential_scan))
			break;
		...
		if (g.fid_filter->HasFIDFilter() && !g.fid_filter->ContainsFID(fid))
			continue;
		if (g.fid_filter->RequiresGenericEvaluation() && !g.fid_filter->EvaluateFID(fid))
			continue;

		// Get full inode metadata
		LustreInode inode;
		if (!l.scanner->ReadInode(ino, inode, g.scan_config))
			continue;
