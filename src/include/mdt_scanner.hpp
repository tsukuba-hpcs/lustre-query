...
public:
	...
	bool ReadInodeLinkEA(ext2_ino_t ino, LustreFID &fid_out, std::vector<LinkEntry> &links_out, bool read_names = true);

	//! Read a specific inode's layout components (for lustre_layouts)
	bool ReadInodeLayouts(ext2_ino_t ino, LustreFID &fid_out, std::vector<LustreLayoutComponent> &components);
	...
	bool ReadInodeLayoutObjects(ext2_ino_t ino, LustreFID &fid_out, std::vector<LustreLayoutComponent> &components,
	                            std::vector<LustreOSTObject> &objects, const MDTScanConfig &config = MDTScanConfig {});
	...
	bool ReadInodeObjects(ext2_ino_t ino, LustreFID &fid_out, std::vector<LustreOSTObject> &objects);
	...
	bool GetNextInodeObjects(LustreFID &fid_out, std::vector<LustreOSTObject> &objects, const MDTScanConfig &config,
	                         ext2_ino_t max_ino);
...
private:
	...
	bool ReadBufferedXattrPrefix(uint8_t name_index, const char *short_name, size_t short_name_len, void *scratch,
	                             unsigned int wanted, const uint8_t *&value_ptr, size_t &value_len);
