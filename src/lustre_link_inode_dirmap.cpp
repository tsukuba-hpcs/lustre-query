...
static const vector<string> LID_COLUMN_NAMES = {
    "fid", "parent_fid", "name", "link_device",
    "dir_fid", "dirmap_parent_fid", "dir_device", "parent_device",
    "master_mdt_index", "stripe_index", "stripe_count", "hash_type", "layout_version", "source",
    "inode_fid", "ino", "type", "mode", "nlink", "uid", "gid",
    "size", "blocks", "atime", "mtime", "ctime", "projid", "flags", "inode_device"
};
...
struct LIDLocalState : public LocalTableFunctionState {
    ...
    bool LookupFIDCrossMDT(const LustreFID &fid, ext2_ino_t &ino_out, idx_t &scanner_idx) {
        for (idx_t i = 0; i < resolve_scanners.size(); i++) {
            if (resolve_scanners[i]->LookupFID(fid, ino_out)) { scanner_idx = i; return true; }
        }
        return false;
    }
...
static unique_ptr<FunctionData> BindMulti(ClientContext &, TableFunctionBindInput &input,
    ...
    if (r->device_paths.empty()) throw BinderException("lustre_link_inode_dirmap requires at least one device path");
    ParseNamedParameters(input.named_parameters, r->scan_config);
    names = LID_COLUMN_NAMES;
    ret = LID_COLUMN_TYPES;
    return std::move(r);
...
static void WriteRow(DataChunk &output, idx_t row, const vector<idx_t> &cols,
    ...
    for (idx_t i = 0; i < cols.size(); i++) {
        ...
        case 18: if (inode_ok) FlatVector::GetData<uint32_t>(vec)[row] = dir_inode.nlink; else FlatVector::SetNull(vec, row, true); break;
        case 19: if (inode_ok) FlatVector::GetData<uint32_t>(vec)[row] = dir_inode.uid; else FlatVector::SetNull(vec, row, true); break;
        case 20: if (inode_ok) FlatVector::GetData<uint32_t>(vec)[row] = dir_inode.gid; else FlatVector::SetNull(vec, row, true); break;
        case 21: if (inode_ok) FlatVector::GetData<uint64_t>(vec)[row] = dir_inode.size; else FlatVector::SetNull(vec, row, true); break;
        case 22: if (inode_ok) FlatVector::GetData<uint64_t>(vec)[row] = dir_inode.blocks; else FlatVector::SetNull(vec, row, true); break;
        case 23: if (inode_ok) FlatVector::GetData<timestamp_t>(vec)[row] = Timestamp::FromEpochSeconds(dir_inode.atime); else FlatVector::SetNull(vec, row, true); break;
        case 24: if (inode_ok) FlatVector::GetData<timestamp_t>(vec)[row] = Timestamp::FromEpochSeconds(dir_inode.mtime); else FlatVector::SetNull(vec, row, true); break;
...
static const vector<string> &GetNames() { static const vector<string> n = LID_COLUMN_NAMES; return n; }
static const vector<LogicalType> &GetTypes() { static const vector<LogicalType> t = LID_COLUMN_TYPES; return t; }

const vector<string> &LustreLinkInodeDirMapFunction::GetColumnNames() { return GetNames(); }
const vector<LogicalType> &LustreLinkInodeDirMapFunction::GetColumnTypes() { return GetTypes(); }

TableFunction LustreLinkInodeDirMapFunction::GetFunction(bool multi) {
    if (multi) {
        TableFunction f("lustre_link_inode_dirmap", {LogicalType::LIST(LogicalType::VARCHAR)}, Execute, BindMulti, InitGlobal, InitLocal);
        SetProps(f); return f;
    }
