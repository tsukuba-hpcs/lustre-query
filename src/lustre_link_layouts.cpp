...
static constexpr idx_t LINK_LAYOUT_EXACT_LOOKUP_BATCH_SIZE = 512;
...
struct LustreLinkLayoutsFilter {
	...
	bool HasAnyFilter() const {
		return (link_filter && link_filter->HasAnyFilter()) || (name_predicate && name_predicate->HasPredicate()) ||
		       ...
		       (layout_device_predicate && layout_device_predicate->HasPredicate()) || !layout_filters.empty();
	}

	bool UseSequentialScan() const {
		return lookup_fids.empty() || lookup_fids.size() > LINK_LAYOUT_SEQ_SCAN_THRESHOLD;
	}
	...
	void RebuildLookupFIDs() {
		...
		if (has_link_fids && has_layout_fids) {
			lookup_fids.reserve(MinValue(link_filter->fid_values.size(), layout_fid_filter->fid_values.size()));
	...
	Value GetLayoutValue(idx_t actual_column_idx, const LustreLayoutComponent &layout, const string &device_path) const {
		...
		case 3:
			return Value();
		case 4:
			return Value::UINTEGER(layout.comp_index);
		case 5:
			return Value::UINTEGER(layout.comp_id);
		case 6:
			return Value::USMALLINT(layout.mirror_id);
		case 7:
			return Value::UINTEGER(layout.comp_flags);
		case 8:
			return Value::UBIGINT(layout.extent_start);
		case 9:
			return Value::UBIGINT(layout.extent_end);
		case 10:
			return Value::UINTEGER(layout.pattern);
		case 11:
			return Value::UINTEGER(layout.stripe_size);
		case 12:
			return Value::USMALLINT(layout.stripe_count);
		case 13:
			return Value::USMALLINT(layout.stripe_offset);
		case 14:
			return layout.pool_name.empty() ? Value() : Value(layout.pool_name);
		case 15:
			return Value::UTINYINT(layout.dstripe_count);
		case 16:
			return Value::UTINYINT(layout.cstripe_count);
		case 17:
			return Value::UTINYINT(layout.compr_type);
		case 18:
			return Value::UTINYINT(layout.compr_lvl);
		case 19:
	...
	bool MatchesFID(const LustreFID &fid) const {
		...
		if (layout_fid_filter) {
			if (!layout_fid_filter->ContainsFID(fid)) {
				return false;
			}
			if (layout_fid_filter->RequiresGenericEvaluation() && !layout_fid_filter->EvaluateFID(fid)) {
				return false;
			}
		}
		return true;
	...
	bool MatchesLayout(const LustreFID &fid, const LustreLayoutComponent &layout, const string &device_path) const {
		...
		for (const auto &entry : layout_filters) {
			...
			if (!EvaluateTableFilterValue(*entry.second, GetLayoutValue(entry.first, layout, device_path))) {
				return false;
			}
		}
		return true;
	...
	static unique_ptr<LustreLinkLayoutsFilter> Create(const TableFilterSet *filters, const vector<idx_t> &column_ids) {
		auto result = make_uniq<LustreLinkLayoutsFilter>();
		...
		if (filters) {
			for (const auto &entry : filters->filters) {
				...
				case 19:
					...
					break;
				default:
					if (actual_column_idx >= 3 && actual_column_idx <= 18) {
		...
		result->layout_fid_filter = FIDOnlyFilter::Create(filters, column_ids, 3);
		result->RebuildLookupFIDs();
		return result;
	}
};

struct LustreLinkLayoutsGlobalState : public GlobalTableFunctionState {
	vector<string> device_paths;
	vector<idx_t> column_ids;
	MDTScanConfig scan_config;
	unique_ptr<LustreLinkLayoutsFilter> filters;
...
struct LustreLinkLayoutsLocalState : public LocalTableFunctionState {
	...
	vector<LustreLinkLayoutRow> pending_results;
	idx_t pending_results_idx = 0;

	LustreLinkLayoutsLocalState() {
		scanner = make_uniq<MDTScanner>();
	}
};

const vector<string> &LustreLinkLayoutsFunction::GetColumnNames() {
	return LINK_LAYOUT_COLUMN_NAMES;
}

const vector<LogicalType> &LustreLinkLayoutsFunction::GetColumnTypes() {
	return LINK_LAYOUT_COLUMN_TYPES;
}
...
static unique_ptr<FunctionData> LustreLinkLayoutsBindSingle(ClientContext &context, TableFunctionBindInput &input,
	...
	names = LustreLinkLayoutsFunction::GetColumnNames();
	return_types = LustreLinkLayoutsFunction::GetColumnTypes();
...
static unique_ptr<FunctionData> LustreLinkLayoutsBindMulti(ClientContext &context, TableFunctionBindInput &input,
	...
	if (result->device_paths.empty()) {
		throw BinderException("lustre_link_layouts_internal requires at least one device path");
	}
	ParseNamedParameters(input.named_parameters, result->scan_config);
	EstimateCardinality(*result);
	names = LustreLinkLayoutsFunction::GetColumnNames();
...
static bool InitializeLinkLayoutsDevice(LustreLinkLayoutsGlobalState &gstate, idx_t device_idx) {
...
static unique_ptr<GlobalTableFunctionState> LustreLinkLayoutsInitGlobal(ClientContext &context,
                                                                        TableFunctionInitInput &input) {
	auto &bind_data = input.bind_data->Cast<LustreQueryBindData>();
	auto result = make_uniq<LustreLinkLayoutsGlobalState>();
	result->device_paths = bind_data.device_paths;
	result->column_ids = input.column_ids;
	result->scan_config = bind_data.scan_config;
	result->filters = LustreLinkLayoutsFilter::Create(input.filters.get(), input.column_ids);
	...
	if (!result->device_paths.empty()) {
		InitializeLinkLayoutsDevice(*result, 0);
	}
	return std::move(result);
}

static unique_ptr<LocalTableFunctionState> LustreLinkLayoutsInitLocal(ExecutionContext &context,
                                                                      TableFunctionInitInput &input,
                                                                      GlobalTableFunctionState *global_state) {
	return make_uniq<LustreLinkLayoutsLocalState>();
}

static void WriteLinkLayoutColumn(Vector &vec, idx_t col_idx, idx_t row_idx, const LustreLinkLayoutRow &row,
...
static void ExpandLinkLayoutRows(vector<LustreLinkLayoutRow> &out, const LustreFID &fid, const vector<LinkEntry> &links,
                                 const vector<LustreLayoutComponent> &layouts, const LustreLinkLayoutsFilter *filters,
                                 const string &device_path) {
	if (links.empty() || layouts.empty()) {
		return;
	}
	...
	vector<const LustreLayoutComponent *> filtered_layouts;
	filtered_layouts.reserve(layouts.size());
	for (const auto &layout : layouts) {
		if (!filters || filters->MatchesLayout(fid, layout, device_path)) {
			filtered_layouts.push_back(&layout);
		}
	}
	if (filtered_layouts.empty()) {
		return;
	}

	for (auto link : filtered_links) {
		for (auto layout : filtered_layouts) {
			LustreLinkLayoutRow row;
			row.fid = fid;
...
static void ResetLinkLayoutsBlockGroupState(LustreLinkLayoutsLocalState &lstate) {
...
static bool EnsureLinkLayoutsScanner(LustreLinkLayoutsGlobalState &gstate, LustreLinkLayoutsLocalState &lstate,
	...
	if (lstate.scanner_initialized) {
		...
		ResetLinkLayoutsBlockGroupState(lstate);
	...
	if (!gstate.device_initialized.load()) {
		...
		if (!gstate.device_initialized.load()) {
			if (!InitializeLinkLayoutsDevice(gstate, dev_idx)) {
				return false;
			}
		}
	}
...
static bool ExecuteExactFIDPath(LustreLinkLayoutsGlobalState &gstate, LustreLinkLayoutsLocalState &lstate,
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		while (lstate.pending_results_idx < lstate.pending_results.size() && output_count < STANDARD_VECTOR_SIZE) {
			...
			for (idx_t i = 0; i < gstate.column_ids.size(); i++) {
				WriteLinkLayoutColumn(output.data[i], gstate.column_ids[i], output_count, row, current_device);
			}
		...
		lookup_batch.reserve(LINK_LAYOUT_EXACT_LOOKUP_BATCH_SIZE);
		while (lookup_batch.size() < LINK_LAYOUT_EXACT_LOOKUP_BATCH_SIZE) {
		...
		for (auto &entry : lookup_batch) {
			...
			if (!lstate.scanner->ReadInodeLinkLayouts(entry.ino, fid, links, layouts, gstate.scan_config)) {
				continue;
			}
			ExpandLinkLayoutRows(lstate.pending_results, fid, links, layouts, gstate.filters.get(), current_device);
		}
	}
...
static bool ExecuteSequentialPath(LustreLinkLayoutsGlobalState &gstate, LustreLinkLayoutsLocalState &lstate,
                                  DataChunk &output, idx_t &output_count) {
	const auto &current_device = lstate.initialized_device_path;
	while (output_count < STANDARD_VECTOR_SIZE) {
		while (lstate.pending_results_idx < lstate.pending_results.size() && output_count < STANDARD_VECTOR_SIZE) {
		...
		if (!ClaimNextLinkLayoutsBlockGroup(gstate, lstate)) {
			return false;
		}
		...
		while (output_count < STANDARD_VECTOR_SIZE) {
			...
			vector<LinkEntry> links;
			vector<LustreLayoutComponent> layouts;
			if (!lstate.scanner->GetNextInodeLinkLayouts(fid, links, layouts, gstate.scan_config,
...
static void LustreLinkLayoutsExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &gstate = data_p.global_state->Cast<LustreLinkLayoutsGlobalState>();
	auto &lstate = data_p.local_state->Cast<LustreLinkLayoutsLocalState>();
	...
	while (output_count < STANDARD_VECTOR_SIZE) {
		if (!EnsureLinkLayoutsScanner(gstate, lstate, gstate.use_sequential_scan)) {
			break;
		}
...
static unique_ptr<NodeStatistics> LustreLinkLayoutsCardinality(ClientContext &context,
...
TableFunction LustreLinkLayoutsFunction::GetFunction(bool multi_device) {
	...
	func.cardinality = LustreLinkLayoutsCardinality;
	return func;
}

TableFunctionSet LustreLinkLayoutsFunction::GetFunctionSet() {
	TableFunctionSet set("lustre_link_layouts_internal");
